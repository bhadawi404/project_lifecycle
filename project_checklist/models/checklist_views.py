...
class ProjectChecklistViewPage(models.Model):
    _name = 'project.checklist.view.page'
    _description = 'Checklist Lines per Page'
    _auto = False
    _rec_name = 'line_name'
    ...
    page_id = fields.Many2one('project.page', string='Project Page', readonly=True)
    line_name = fields.Char(string='Checklist Item', readonly=True)
    status = fields.Selection([
        ('todo', 'To Do'),
        ('in_progress', 'In Progress'),
        ...
        self.env.cr.execute(f"""
            CREATE OR REPLACE VIEW {self._table} AS (
                SELECT
                    ...
                    cl.completed_on
                FROM project_checklist_line cl
                JOIN checklist_line_project_page_rel rel
                    ON rel.cll_id = cl.id
                JOIN project_page p
                    ON p.id = rel.project_page_id
            )
        """)
